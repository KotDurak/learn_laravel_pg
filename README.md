...
**Note**: Your postgress database host name should be `postgress`, **not** `localhost`. The username and database should both be `homestead` with a password of `secret`. 
...
## Persistent Postgress Storage

By default, whenever you bring down the Docker network, your PG data will be removed after the containers are destroyed. If you would like to have persistent data that remains after bringing containers down and back up, do the following:

1. Create a `postgress` folder in the project root, alongside the `nginx` and `src` folders.
2. Under the postgress service in your `docker-compose.yml` file, add the following lines:
